...
async def chat(req: Request):
    ...
    async with httpx.AsyncClient(headers=_auth_headers(), timeout=120) as client:
        ...
        async for event in a2a_client.send_message(msg):
            ...
            elif getattr(getattr(update, "status", None), "message", None):
                msg_obj = update.status.message
                if getattr(msg_obj, "role", None) == Role.agent or getattr(msg_obj, "role", None) == "agent":
                    extracted = _extract_parts(msg_obj.parts)
                    if extracted:
                        parts.extend(extracted)

        # Fallback: pull parts from final task history or artifacts if no parts extracted yet
            ...
            if getattr(last_task, "history", None):
                for m in reversed(last_task.history):
                    if getattr(m, "role", None) == Role.agent or getattr(m, "role", None) == "agent":
                        extracted = _extract_parts(m.parts)
                        if extracted and "Cannot add session to memory" not in str(extracted):
                            parts.extend(extracted)
                            break
